...
export default function Mandelbrot({ width = 800, height = 600, maxIterations = 100, colorScheme = 0, onZoomChange }: Props) {
  ...
  const [view, setView] = useState({ centerX: -0.75, centerY: 0, scale: 1 })
  ...
  const draw = useCallback(() => {
    ...
    const viewWidth = 3 / view.scale
    const viewHeight = 3 / view.scale
  ...
  const handleReset = useCallback(() => {
    setView({ centerX: -0.75, centerY: 0, scale: 1 })
